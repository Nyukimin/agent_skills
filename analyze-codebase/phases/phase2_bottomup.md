...
Phase 1 の成果物を実コードと突合せて修正・補強し、全体概要を最終化したうえで、潜在バグ・設計書乖離等の異常一覧を生成する。

## 入力

- Phase 1 の出力（{out}/modules/*.md, 結合ポイントマップ.md, ユースケース逆引き.md, アーキテクチャ総合.md）
- manifest.json（phase_done が Phase 1 完了以上であること）
- プロファイル（異常一覧のファイル名は output.anomaly_file で指定可）

## 出力

- {out}/modules/ 配下のモジュール解析の上書き更新
- {out}/結合ポイントマップ.md, {out}/ユースケース逆引き.md の上書き更新
- {out}/アーキテクチャ総合.md の最終版
- {out}/modules/潜在バグ一覧.md（またはプロファイルで指定した異常一覧ファイル名）

## 前提

- 実行順序は [pipeline.yaml](../pipeline.yaml) を参照する。Step 9 完了後に 10 → 11 → 12 → 13 を実行する。
- Phase 1 の出力が {out} に存在し、manifest の phase_done が Phase 1 完了以上であること。満たさない場合は SKILL の「Phase 判定フロー」に従いエラー終了する。
...
各モジュール解析ファイル（プロファイルの module_groups の数だけ）に対して:
...
### 出力
- {out}/結合ポイントマップ.md を上書き更新
- {out}/ユースケース逆引き.md を上書き更新
...
1. **全モジュール解析（プロファイルの module_groups の数だけ）+ アーキテクチャ総合を読み込む**
2. **以下のセクションから項目を抽出**:
...
### 出力
- {out}/modules/潜在バグ一覧.md（またはプロファイルの output.anomaly_file）
...
- 生成・更新したファイルには frontmatter と概要・関連ドキュメントを維持すること。
